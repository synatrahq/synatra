...
| 1     | First-level subagent                          |
| 2+    | Nested subagent (if allowed)                  |
...
Token usage is aggregated per run, per thread, per environment, and per organization.
...
| --------------------- | ------------------------------------- | ---------- |
| Max iterations        | Maximum LLM calls per run             | 10         |
| Max tool calls        | Max parallel tool calls per iteration | 10         |
| Max active time       | Maximum execution time                | 10 minutes |
| Human request timeout | Max wait for human input              | 72 hours   |
