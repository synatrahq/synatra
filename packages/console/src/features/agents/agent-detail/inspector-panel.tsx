...
import { DiffInspector } from "./inspector/diff-inspector"
import { ResourceConnectionWizard } from "./inspector/resource-connection-wizard"
import { TriggerRequestWizard } from "./inspector/trigger-request-wizard"
...
import { OnboardingVideoInspector } from "./inspector/onboarding-video-inspector"
...
function getTabIcon(tab: TabItem) {
  ...
  if (tab.type === "onboarding_video") return VideoCamera
  return Brain
}
...
function getTabIconClass(tab: TabItem): string {
  ...
  if (tab.type === "onboarding_video") return "text-accent"
  ...
  return (
    <div class="flex h-full flex-col bg-surface-elevated">
      ...
      <div class="flex-1 overflow-y-auto scrollbar-thin">
        <Show when={activeTab()?.type === "onboarding_video"}>
          <OnboardingVideoInspector onOpenModal={() => props.onOpenVideoModal?.()} />
        </Show>
        <Show when={activeTab()?.type !== "onboarding_video"}>
          <Show
